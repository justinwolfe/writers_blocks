...
var wordTargetPercentage;
 ...
 $(document).ready(function() {
	prepareOblique();
	//insert function to check for saved settings in localStorage
	...
	$("#processSpace").keydown(function(event){
		...
		if ((keyPressed >= 48 && keyPressed <= 90) || (keyPressed >= 186 && keyPressed <= 222)){
			...
			if (menuDisplayed == false){
				$("#menuDiv").append("\
					<div id='menuDisplay'>\
						<div id='wordTargetContainer'>word count target</div>\
						<div id='wordTargetContainer'>change screen color toward target? COLOR</div>\
					</div>");
